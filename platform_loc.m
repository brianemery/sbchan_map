function [hp,plat] = platform_loc(swch)
% PLATFORM_LOC.M
% [hp,plat] = platform_loc(swch)
...
%
% Use this to just get a struct with the platform data:
% [~,plat] = platform_loc;

% Circa 1990s by Brian Emery

% input logic:
if nargin == 0
    noplot = true; hp =[]; %  % default NO PLOT
else
    noplot = false;
end
...
if ~noplot
    ...
    for i = 1:numel(site)
        ...
        hi=plot(site(i).lon,site(i).lat,'ks');

        set(hi,'Clipping','on','MarkerSize',3,'MarkerFaceColor',[.5 .5 .5]);
